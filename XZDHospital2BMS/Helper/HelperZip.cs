...
namespace Helper
{
  ...
  class HelperZip
  {
    ...
    /// 压缩文件   
    /// </summary>   
    /// <param name="strFileToZip">要压缩的文件全名</param>   
    /// <param name="strFileZiped">压缩后的文件名</param>   
    /// <param name="strPassword">密码</param>   
    /// <returns>压缩结果</returns>   
    public bool ZipFile(string strFileToZip, string strFileZiped, string strPassword)
    {
      bool boolResult = true;
      ZipOutputStream objOSZip = null;
      FileStream objFS = null;
      ZipEntry objEntryZip = null;
      if (!File.Exists(strFileToZip)) return false;
      try
      {
        objFS = File.OpenRead(strFileToZip);
        byte[] buffer = new byte[objFS.Length];
        objFS.Read(buffer, 0, buffer.Length);
        objFS.Close();

        objFS = File.Create(strFileZiped);
        objOSZip = new ZipOutputStream(objFS);
        if (!string.IsNullOrEmpty(strPassword)) objOSZip.Password = strPassword;
        objEntryZip = new ZipEntry(Path.GetFileName(strFileToZip));
        objOSZip.PutNextEntry(objEntryZip);
        objOSZip.SetLevel(6);

        objOSZip.Write(buffer, 0, buffer.Length);
      ...
      finally
      {
        ...
        if (objEntryZip != null) objEntryZip = null;
        if (objFS != null)
        {
          objFS.Close();
          objFS.Dispose();
